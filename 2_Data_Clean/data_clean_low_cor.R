...
# get rid of the higly correlated columns.
# first get the index of highly correlated columns. 
# add 1 here because the Churn target variable is in the first column. 
high_cor_index=(churn_impute1_dummy%>%select(-Churn)%>%cor%>%findCorrelation(cutoff = 0.9))+1
...
rm(index1,index2,intermediate_churn,churn_impute1_dummy,high_cor_index)
